...
``get-ingress-data`` action. The configurator's reconciliation requires both
``domain`` and ``tunnel-token`` before it publishes or sends tunnel data.

For more information about Juju events, refer to the
